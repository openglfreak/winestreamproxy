...
BOOL prepare_socket(logger_instance* const logger, char const* const socket_path, socket_data* const _socket)
{
    ...
    LOG_TRACE(logger, (TEXT("Preparing socket")));

    socket_path_len = strlen(socket_path);
    if (socket_path_len > sizeof(_socket->addr.sun_path) - 1)
    {
        LOG_CRITICAL(logger, (TEXT("Socket path too long")));
        return FALSE;
    }
    ...
    {
        LOG_CRITICAL(logger, (TEXT("Failed to create eventfd: Error %d"), errno));
        return FALSE;
    }
    ...
    {
        LOG_CRITICAL(logger, (TEXT("Failed to create socket: Error %d"), errno));
        close(_socket->eventfd);
        return FALSE;
    }

    LOG_TRACE(logger, (TEXT("Prepared socket")));
...
{
    LOG_TRACE(logger, (TEXT("Discarding prepared socket")));

    close(socket->eventfd);
    close(socket->socketfd);

    LOG_TRACE(logger, (TEXT("Discarded prepared socket")));
}

BOOL connect_socket(logger_instance* const logger, socket_data* const socket)
{
    LOG_TRACE(logger, (TEXT("Connecting socket")));

    if (connect(socket->socketfd, (struct sockaddr*)&socket->addr, sizeof(socket->addr)) != 0)
    {
        LOG_ERROR(logger, (TEXT("Failed to connect to socket: Error %d"), errno));
        return FALSE;
    }

    LOG_TRACE(logger, (TEXT("Connected socket")));
...
static BOOL signal_eventfd(logger_instance* const logger, int const eventfd)
{
    ...
    {
        LOG_ERROR(logger, (TEXT("Could not send exit signal to socket thread")));
        return FALSE;
    }
...
void close_socket(connection_data* const conn, BOOL const exit_thread)
{
    ...
    LOG_TRACE(conn->proxy->logger, (TEXT("Closing socket")));

    if (exit_thread)
        if (signal_eventfd(conn->proxy->logger, conn->data.socket.eventfd))
            wait_for_thread(conn->proxy->logger, TEXT("socket"), conn->threads.socket);
    ...
    LOG_TRACE(conn->proxy->logger, (TEXT("Closed socket")));
}
